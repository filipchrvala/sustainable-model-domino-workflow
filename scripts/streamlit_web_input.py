...
def _trigger_operational_refresh(state: dict) -> None:
    save_state(STATE_PATH, state)
    st.session_state.form_state = state
    st.session_state.pending_operational_refresh = True
    st.rerun()


def _render_ingest_upload(state: dict, *, key_prefix: str) -> None:
    ...
    data_cfg["append_new_rows_to_company_drop"] = st.checkbox(
        ...
        value=bool(data_cfg.get("append_new_rows_to_company_drop", False)),
        help=H["append_to_drop"],
        key=f"{key_prefix}_append_cb",
    )
    append_up = st.file_uploader(
        "Nová spotreba – CSV na doplnenie",
        type=["csv"],
        key=f"{key_prefix}_append_up",
        help=H["append_csv"],
    )
    if append_up is not None:
        ...
        st.caption(f"Pripravené na ingest: {P.WEB_APPEND_DROP_CSV.name}")

    P.SUSTAINABLE_UPDATES_DIR.mkdir(parents=True, exist_ok=True)
    drop_files = sorted(P.SUSTAINABLE_UPDATES_DIR.glob("*.csv"))
    st.caption(f"Súbory v `company_drop`: {len(drop_files)}")
    if drop_files:
        st.caption(", ".join(p.name for p in drop_files[:5]) + ("…" if len(drop_files) > 5 else ""))


def _render_ingest_refresh_button(state: dict, *, key_prefix: str, compact: bool = False) -> None:
    if compact:
        st.caption("Len ingest, doučenie, forecast a alerty — bez simulácie FVE/batérie.")
    else:
        st.caption(
            "Po nahratí CSV kliknite **Obnoviť data + alerty**. "
            "Nespúšťa celú simuláciu."
        )
    ...
    with btn_col:
        if st.button(
            ...
            help=H["btn_operational_refresh"],
        ):
            _trigger_operational_refresh(state)


def _render_ingest_section(state: dict, *, key_prefix: str, compact: bool = False) -> None:
    _render_ingest_upload(state, key_prefix=key_prefix)
    _render_ingest_refresh_button(state, key_prefix=key_prefix, compact=compact)
...
if st.session_state.pop("pending_operational_refresh", False):
    save_state(STATE_PATH, state)
    materialize_from_state(state)
    st.header("Obnovujem data + alerty…")
    progress = st.progress(0.0, text="Ingest nových CSV…")
    try:
        with st.spinner("Ingest, doučenie modelu, forecast, alerty (bez celej simulácie)…"):
            from workflow.live_refresh import run_operational_refresh

            progress.progress(0.35, text="Ingest a doučenie modelu…")
        ...
        st.success("Nové dáta spracované — zobrazujem aktualizovaný dashboard.")
    except Exception as exc:
        st.session_state.ui_view = "form"
        st.session_state.last_workflow_error = str(exc)
        st.session_state.last_workflow_traceback = traceback.format_exc()
    st.rerun()
...
if st.session_state.ui_view == "dashboard":
    ...
    with nav1:
        if st.button("← Späť", type="secondary", key="dash_back"):
            st.session_state.ui_view = "form"
            st.rerun()

    with st.expander("Nová spotreba na doučenie modelu", expanded=False):
        _render_ingest_section(state, key_prefix="dash", compact=True)
...
if st.session_state.get("last_workflow_error"):
    st.error(f"Posledný beh workflow zlyhal: {st.session_state.last_workflow_error}")
    with st.expander("Detail chyby", expanded=False):
        st.code(st.session_state.get("last_workflow_traceback", ""))
    if st.button("Skryť chybovú správu", key="clear_wf_err"):
        del st.session_state["last_workflow_error"]
        del st.session_state["last_workflow_traceback"]
        st.rerun()
...
with tab_data:
    ...
    _render_ingest_upload(state, key_prefix="form")
    ...
    _render_ingest_refresh_button(state, key_prefix="form", compact=False)
...
with tab_save:
    ...
    st.divider()
    st.markdown("**Doučenie modelu (rýchle):**")
    st.caption(
        "Po nových dátach v záložke „Spotreba a ceny“ použite **Obnoviť data + alerty** — "
        "nie „Spustiť celý workflow“."
    )
    st.divider()
