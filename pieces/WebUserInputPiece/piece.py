        ...
        try:
            ...
            if not state_path.is_file():
                raise FileNotFoundError(
                    f"Missing {state_path}. Open WebUserInputPiece UI in Domino, or run locally: "
                    "streamlit run pieces/WebUserInputPiece/piece.py — ulož formulár, "
                    "potom skopíruj web_form_state.json do WebUserInputPiece_Input."
                )
...
_P_ROOT = Path(__file__).resolve().parents[2]
_P_TESTS = _P_ROOT / "tests"
P = SimpleNamespace(
    PROJECT_ROOT=_P_ROOT,
    TESTS_DIR=_P_TESTS,
    USER_INPUT_DIR=_P_TESTS / "user_input",
    WORKFLOW_USER_INPUT_JSON=_P_TESTS / "user_input" / "workflow_user_input.json",
    USER_SCENARIO_YML=_P_TESTS / "user_input" / "scenario.yaml",
    IN_WEB_USER_INPUT=_P_TESTS / "WebUserInputPiece_Input",
    OUT_WEB_USER_INPUT=_P_TESTS / "WebUserInputPiece_Output",
    WEB_FORM_STATE_JSON=_P_TESTS / "WebUserInputPiece_Input" / "web_form_state.json",
    WEB_UPLOAD_LOAD_CSV=_P_TESTS / "WebUserInputPiece_Input" / "uploaded_load.csv",
    WEB_UPLOAD_PRICES_CSV=_P_TESTS / "WebUserInputPiece_Input" / "uploaded_prices.csv",
    WEB_APPEND_DROP_CSV=_P_TESTS / "WebUserInputPiece_Input" / "pending_append_to_drop.csv",
    GENERATED_DIR=_P_TESTS / "_generated",
    GENERATED_SOLAR_CONFIG_YML=_P_TESTS / "_generated" / "solar_config.yml",
    GENERATED_INVESTMENT_CONFIG_YML=_P_TESTS / "_generated" / "investment_config.yml",
    GENERATED_SCENARIO_YML=_P_TESTS / "_generated" / "scenario.yml",
    GENERATED_BATTERY_CONFIG_YML=_P_TESTS / "_generated" / "battery_config.yml",
    IN_FETCH=_P_TESTS / "FetchEnergyDataPiece_Inputs",
    OUT_USER_INPUT=_P_TESTS / "UserInputPiece_Output",
    SUSTAINABLE_DIR=_P_TESTS / "sustainable",
    SUSTAINABLE_UPDATES_DIR=_P_TESTS / "sustainable" / "company_drop",
)
...
def _load_workflow_user_input() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    return data.get("project_options") or {}, constraints, economics, extras


def _materialize_scenario_config(extras: dict[str, Any]) -> None:
...
def _materialize_optional_configs(extras: dict[str, Any]) -> None:
    P.GENERATED_DIR.mkdir(parents=True, exist_ok=True)
    _materialize_scenario_config(extras)
...
def default_state() -> dict[str, Any]:
    ...
    if P.USER_SCENARIO_YML.is_file():
        ...
        if prod.get("prediction_days") is not None:
            state["scenario"]["prediction_days"] = int(prod["prediction_days"])
...
def build_workflow_user_input_json(state: dict[str, Any]) -> dict[str, Any]:
    ...
    prediction_days = int(scen.get("prediction_days") or 30)
    timestep_minutes = int(scen.get("timestep_minutes") or 15)
...
def build_scenario_yaml(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if scen.get("prediction_days") is not None:
        merged["prediction_days"] = int(scen["prediction_days"])
    return merged
...
def materialize_from_state(
    ...
    *,
    ...
    _po, _c, _e, extras = _load_workflow_user_input()
    _materialize_optional_configs(extras)
...
# --- Streamlit UI (streamlit run pieces/WebUserInputPiece/piece.py) ---

def render_streamlit_web_input() -> None:
    """Web formulár pre WebUserInputPiece — spusti cez streamlit run na tomto súbore."""
    import json
    import sys
    from pathlib import Path
    ...
    from pieces.WebUserInputPiece.models import FIELD_LABELS, H, InputModel as WebInput
    ...
    with tab_scen:
        ...
        with s1:
            ...
            scenario["prediction_days"] = int(
                st.number_input(
                    "Horizont predikcie (dni)",
                    value=int(scenario.get("prediction_days", 30)),
                    min_value=1,
                    max_value=366,
                    help=H["prediction_days"],
                )
            )
    ...
    with tab_save:
        ...
        with col_c:
            if st.button(
                "Uložiť + materializovať pre Domino",
                use_container_width=True,
                help=H["btn_run_workflow"],
            ):
                save_state(STATE_PATH, state)
                paths = materialize_from_state(state)
                st.success("Pripravené pre WebUserInputPiece v Domino workflow.")
                st.json(paths)

        st.info(
            "Celý workflow spúšťaj v **Domino UI**. "
            "Po uložení umiestni web_form_state.json do OneData / WebUserInputPiece_Input.",
            icon="ℹ️",
        )

if __name__ == "__main__":
    render_streamlit_web_input()
